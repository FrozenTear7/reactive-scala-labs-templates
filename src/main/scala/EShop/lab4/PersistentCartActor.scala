...
class PersistentCartActor(
  ...
  def nonEmpty(cart: Cart, timer: Cancellable): Receive = LoggingReceive.withLabel("nonEmpty") {
    case AddItem(item) => persist(ItemAdded(item, cart))(event => updateState(event, Some(timer)))
    ...
    case StartCheckout =>
      val persistentCheckoutActor = context.actorOf(PersistentCheckout.props(self, "persistentCheckoutId"), "checkout")
  ...
  def inCheckout(cart: Cart): Receive = LoggingReceive.withLabel("inCheckout") {
    case CloseCheckout  => persist(CheckoutClosed)(event => updateState(event))
    case CancelCheckout => persist(CheckoutCancelled(cart))(event => updateState(event))
  }
