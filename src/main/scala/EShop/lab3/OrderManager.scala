...
class OrderManager extends Actor {
  ...
  def inCheckout(cartActorRef: ActorRef, senderRef: ActorRef): Receive =
    LoggingReceive.withLabel("In checkout started") {
      case CartActor.CheckoutStarted(checkoutRef, _) =>
        senderRef ! Done
        context become inCheckout(checkoutRef)
    }

  def inCheckout(checkoutActorRef: ActorRef): Receive = LoggingReceive.withLabel("inCheckout") {
  ...
  def inPayment(senderRef: ActorRef): Receive = LoggingReceive.withLabel("inPayment") {
    case Checkout.PaymentStarted(paymentRef) =>
      senderRef ! Done
      context become inPayment(paymentRef, senderRef)
  }

  def inPayment(paymentActorRef: ActorRef, senderRef: ActorRef): Receive = LoggingReceive.withLabel("inPayment") {
    case Pay =>
